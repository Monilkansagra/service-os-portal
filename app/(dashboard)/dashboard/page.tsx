...
import {
    Users, Building2, Server, Activity, Printer, Calendar, RefreshCcw, ArrowUpRight, ArrowDownRight
} from "lucide-react";
...
export default function AdminDashboardPage() {
    ...
    const [metrics, setMetrics] = useState({
        activeUsers: { value: 1240, trend: +12.5, sparkline: [12, 18, 15, 22, 18, 25, 28], color: COLORS.indigo, icon: Users },
        departments: { value: 34, trend: +2.4, sparkline: [30, 32, 32, 33, 33, 34, 34], color: COLORS.violet, icon: Building2 },
        services: { value: 142, trend: -1.2, sparkline: [150, 148, 145, 145, 142, 142, 142], color: COLORS.pink, icon: Server },
        requests: { value: 890, trend: +15.3, sparkline: [70, 80, 95, 85, 110, 120, 135], color: COLORS.emerald, icon: Activity }
    });

    // Simulated Time Filter
    const [timeFilter, setTimeFilter] = useState("7D");

    // Simulated Dynamic Charts
    const [lineChartData, setLineChartData] = useState([
        { name: 'Mon', value: 45, prev: 40 }, { name: 'Tue', value: 52, prev: 48 },
        { name: 'Wed', value: 38, prev: 42 }, { name: 'Thu', value: 65, prev: 50 },
        { name: 'Fri', value: 78, prev: 60 }, { name: 'Sat', value: 40, prev: 35 },
        { name: 'Sun', value: 30, prev: 25 },
    ]);

    const [pieData, setPieData] = useState([
        { name: 'IT Dept', value: 45 }, { name: 'HR', value: 25 },
        { name: 'Facilities', value: 20 }, { name: 'Finance', value: 10 }
    ]);

    const barData = [
        { name: 'Network', time: 1.2 }, { name: 'Hardware', time: 3.5 },
        { name: 'Software', time: 0.8 }, { name: 'Access', time: 2.1 },
        { name: 'Maintenance', time: 4.8 }
    ];

    // Simulating live data update every 30s
    useEffect(() => {
        setMounted(true);
        const interval = setInterval(() => {
            setLastUpdate("just now");

            // slightly fluctuate data
            setLineChartData(prev => prev.map(d => ({
                ...d,
                value: d.value + Math.floor(Math.random() * 10 - 5)
            })));
    ...
    // Update timer strictly for "Last updated: X min ago"
    useEffect(() => {
        let minutes = 0;
            ...
            >
                ...
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 uppercase tracking-widest">ADMIN</span>
                    </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                >
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex bg-[#13131F] border border-indigo-900/40 rounded-lg p-1">
